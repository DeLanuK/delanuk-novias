async function initAuth() {
  // Si venimos de un link de recuperaciÃ³n del mail, mostramos la pantalla de nueva contraseÃ±a
  ...
  sb.auth.onAuthStateChange(async (event, session) => {
    if (event === 'PASSWORD_RECOVERY') {
      ...
      // Si el usuario estÃ¡ en la pantalla de reset, NO lo metemos al app
...
function showLogin() {
  // ð Si llegÃ³ con un link tipo #/novia/42, lo guardamos
  // para llevarla ahÃ­ despuÃ©s de que se loguee.
...
document.body.classList.toggle('is-staff', !window.AppState.isAdmin);
  ...
  // ð¯ Si habÃ­a un destino pendiente (link compartido antes del login),
